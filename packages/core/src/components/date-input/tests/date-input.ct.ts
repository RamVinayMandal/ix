...
regressionTest('required', async ({ mount, page }) => {
  ...
  // Use component's methods to properly trigger validation
...
regressionTest(
  ...
  async ({ mount, page }) => {
    ...
    // Ensure value is synchronized to native input
    await dateInput.evaluate(async (el: HTMLIxDateInputElement) => {
      await el.getNativeInputElement();
      // Force a re-render cycle to ensure value is synced
      el.value = el.value;
    });
...
regressionTest(
  ...
  async ({ page, mount }) => {
    ...
    await dateInput.evaluateHandle((el: HTMLIxDateInputElement) => {
